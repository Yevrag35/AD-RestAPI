...
public sealed partial class DistinguishedNameOld
{
    ...
    /// <summary>
    /// Splits the current <see cref="DistinguishedNameOld"/> into its constituent <see cref="RelativeName"/> components.
    /// </summary>
    /// <returns>
    /// An array of <see cref="RelativeName"/> components that make up the <see cref="DistinguishedNameOld"/>.
    /// </returns>
    ...
    /// <summary>
    /// Attempts to split the current <see cref="DistinguishedNameOld"/> into its constituent <see cref="RelativeName"/> 
    ...
    public static DistinguishedNameOld Join(ReadOnlySpan<RelativeName> relativeNames)
    {
        ...
        DistinguishedNameOld result = new(span.Slice(0, pos), first.Value, span.Slice(first.Value.Length + 1, minusFirst));
