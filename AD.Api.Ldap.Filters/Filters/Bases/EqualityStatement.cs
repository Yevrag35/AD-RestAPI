...
{
    /// <summary>
    /// An <see langword="abstract"/> base statement class that specifies equality in an LDAP filter.
    /// </summary>
    public abstract class EqualityStatement : FilterStatementBase
    {
        ...
        protected static bool TryGetLdapName(MemberExpression expression, out string ldapPropertyName)
        {
            ldapPropertyName = null;
            Type ldapPropertyAttType = typeof(LdapPropertyAttribute);

            if (expression.Member.CustomAttributes.Any(att => ldapPropertyAttType.IsAssignableFrom(att.AttributeType)
