...
public ref partial struct RentedBuffer<T>
{
	...
	private ref struct State
	{
		...
		internal bool ClearOnDispose
		{
			...
			set => _state = (uint)((_state & ~RentedBufferConstants.ClearBit) | Mask(value, RentedBufferConstants.ClearBit));
		}
		internal bool IsRented
		{
			readonly get => (_state & RentedBufferConstants.RentedBit) != 0;
			set => _state = (uint)((_state & ~RentedBufferConstants.RentedBit) | Mask(value, RentedBufferConstants.RentedBit));
		}
		...
		private static uint Mask(bool value, [ConstantExpected] byte bit)
		{
			return (uint)((-Unsafe.As<bool, byte>(ref value)) & bit);
		}
	}
}
